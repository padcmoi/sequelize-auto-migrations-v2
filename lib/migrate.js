...
const reverseSequelizeDefValueType = function (defaultValue, prefix = 'Sequelize.') {
  if (typeof defaultValue === 'object') {
    if (defaultValue.constructor && defaultValue.constructor.name) {
      let name = defaultValue.constructor.name;
...
const reverseModels = function (sequelize, models) {
  ...
  for (let model in models) {
    let attributes = models[model].attributes || models[model].rawAttributes || models[model].tableAttributes;
...
module.exports = {
    ...
    async down(queryInterface, Sequelize) {
      migrationCommands.reverse().forEach(async (el, index) => {
        if (el.fn == "createTable" && typeof el.params[0] == "string") {
          console.log("[#" + index + "] dropTabble: " + el.params[0])
          await queryInterface.dropTable(el.params[0])
        }
      })
    },
    info: info
...
const executeMigration = async function (queryInterface, filename, pos, cb) {
  ...
  return mig.up(queryInterface, Sequelize);
};
